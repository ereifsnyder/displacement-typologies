...
hospitals <- 
    st_join(
        readRDS(paste0(path, '/displacement-typologies/data/inputs/Hospitals.rds')) %>%
            st_as_sf(
                coords = c('X', 'Y'), 
                crs = 4269
            ) %>% 
...
st_write(obj = scag_sf, dsn = paste0(path, "/displacement-typologies/data/downloads_for_public/scag.shp"))
